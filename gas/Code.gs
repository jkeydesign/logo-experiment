...
function appendEvent(ss, data, now) {
  ...
  sheet.appendRow([
    ...
    data.eventName ?? '',
    payload.eventType ?? data.eventName ?? '',
    payload.conditionType ?? '',
    data.condition ?? '',
    payload.conditionLabel ?? data.conditionLabel ?? '',
    payload.conditionOrder ?? payload.condition_order ?? '',
    data.setId ?? '',
    data.stimulusId ?? '',
    payload.logoId ?? data.stimulusId ?? '',
    ...
    payload.previousFinalLogoId ?? '',
    payload.newFinalLogoId ?? '',
    payload.finalLogoId ?? '',
    payload.scoreType ?? '',
    payload.elapsedMs ?? data.sinceExperimentStartMs ?? '',
    payload.conditionStartTime ?? '',
    payload.finalConfirmTime ?? '',
    payload.finalDecisionElapsedMs ?? '',
    payload.isAiRecommended ?? '',
    payload.aiRank ?? '',
    payload.aiScore ?? '',
